...
class Inscription
{
    public static function ajouter($nom, $prenom, $email, $formation_id): int
    {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO inscriptions (nom, prenom, email, formation_id, statut_paiement, date_inscription) VALUES (?, ?, ?, ?, "en_attente", NOW())'
        );
    ...
    public static function getById($id)
    {
        $pdo = Database::connect();
        $stmt = $pdo->prepare(
            'SELECT i.*, f.titre AS formation_titre, f.prix, f.duree, f.niveau FROM inscriptions i JOIN formations f ON i.formation_id = f.id WHERE i.id = ?'
        );
        $stmt->execute([$id]);
        return $stmt->fetch();
    }
